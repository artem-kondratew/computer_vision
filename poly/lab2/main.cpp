...
// /*--------------------------------------------------------------------------------------------------------------------------------*/
    ...
    cv::Mat laplacian_unsharp_mask = ImgProc::unsharpMaskLaplace(gray, 0.5);

    cv::imshow("laplacian_unsharp_mask.jpg", laplacian_unsharp_mask);


// /*--------------------------------------------------------------------------------------------------------------------------------*/

    /* Attempt 2 */
    cv::Mat l = ImgProc::laplacian(gray);
    cv::Mat g;
    cv::GaussianBlur(gray, g, {3, 3}, 0);
    cv::imshow("laplace.jpg", l);
    cv::imshow("gauss.jpg", g);
